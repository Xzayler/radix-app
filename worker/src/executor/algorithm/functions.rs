...
pub struct PreComputed<'a, T: Clone> {
    pub m_inv: DMatrix<T>,
    pub g: DMatrix<T>, // maybe just Vec<T>?
    pub u: DMatrix<T>,
    pub d: &'a Vec<DVector<T>>,
...
pub fn pre_compute<'a>(
    m: DMatrix<f64>,
    d: &'a Vec<DVector<f64>>,
) -> Result<PreComputed<'a, f64>, OpError> {
...
// TODO: Set to provate
pub fn h(u: &DMatrix<f64>, g: &DMatrix<f64>, z: &DVector<f64>) -> Result<i32, OpError> {
    let uz = u * z;
    // println!("u: {:?}", u);
    // println!("z: {:?}", z);
    // println!("uz: {:?}", uz);
...
pub fn get_cover_box(
    ...
    let mut xi: Vec<DVector<f64>> = vec![DVector::from_element(0, 1.0); c];
    let mut eta: Vec<DVector<f64>> = vec![DVector::from_element(0, 1.0); c];
    for j in 0..c {
...
mod tests {
    ...
    fn get_congruent_test() {
        ...
        let data: PreComputed<f64> = match pre_compute(base, &d) {
