...
pub async fn update_db_with_results(
  ...
  let mut transaction = pool.begin().await.map_err(|err| {
    WorkerError::Database("Couldn't start transaction: ".to_string() + err.to_string().as_str())
  })?;
  ...
  if job_res.rows_affected() == 0 {
    return Err(WorkerError::Database(
      "Couldn't find row to update".to_string(),
    ));
  }
...
pub async fn update_db_with_job_error(
  pool: &sqlx::PgPool,
  job_id: i32,
  err: String,
) -> Result<(), WorkerError> {
  ...
  .await
  .map_err(|err| {
    WorkerError::Database(format!("Couldn't update failed job id {job_id}: {err}"))
  })?;
...
pub async fn connect() -> Result<Pool<Postgres>, WorkerError> {
  ...
  PgPoolOptions::new()
    ...
    .await
    .map_err(|err| {
      WorkerError::Database("Could't connect to db: ".to_string() + err.to_string().as_str())
    })
}
